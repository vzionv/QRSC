...
def show(m,msg=''):
    ...
    except OSError: cols=999
    if n>cols: print('terminal too narrow: need %d cols, have %d'%(n,cols))
...
def fileqr(path,once=0):
    ...
    try:
        while i<len(ch):
            ...
            m,v,n=qr(ch[i],0)
            show(m,'chunk %03d/%03d | %d%% | %s | %d bytes | QR v%d | Ctrl+C stop'%(i+1,len(ch),(i+1)*100//len(ch),fn,n,v))
            if once: break
            i+=1
