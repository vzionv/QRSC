...
def show(m,msg=''):
    cls(); q=4; n=len(m); w='\033[47m  \033[0m'; b='\033[40m  \033[0m'
    line=w*(n+q*2); print(line)
    for r in m:
        print(w*q+''.join(b if x else w for x in r)+w*q)
    print(line)
    if msg: print(msg)
    sys.stdout.flush()

def textqr(s):
    m,v,n=qt(s); show(m,'QR v%d | %d bytes | %d chars'%(v,n,len(s)))
...
def fileqr(path,once=0):
    if isc(path):
        ...
        ch,fn=chunks(path); cache=0
    ts=time.strftime('%H%M'); os.makedirs(CD,exist_ok=True)
    i=0
    try:
        while i<len(ch):
            if not cache:
...
def usage():
    print('usage:')
    print('  python3 scqr_term.py -t TEXT')
    print('  echo TEXT | python3 scqr_term.py -')
    print('  python3 scqr_term.py FILE')
    print('  python3 scqr_term.py --once FILE')
    print('  python3 scqr_term.py --clear')
