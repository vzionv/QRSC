#!/usr/bin/env python3
import sys,os,time,shutil,ctypes,ctypes.util
...
class XS(ctypes.Structure):
    _fields_=[('type',ctypes.c_int),('serial',ctypes.c_ulong),('send_event',ctypes.c_int),('display',ctypes.c_void_p),('requestor',ctypes.c_ulong),('selection',ctypes.c_ulong),('target',ctypes.c_ulong),('property',ctypes.c_ulong),('time',ctypes.c_ulong)]
class XE(ctypes.Union):
    _fields_=[('type',ctypes.c_int),('xselection',XS),('pad',ctypes.c_long*24)]

class XClip:
    def __init__(self):
        lib=ctypes.util.find_library('X11') or 'libX11.so.6'
        x=ctypes.CDLL(lib); self.x=x
        ...
        x.XPending.argtypes=[ctypes.c_void_p]; x.XPending.restype=ctypes.c_int
        x.XNextEvent.argtypes=[ctypes.c_void_p,ctypes.POINTER(XE)]
        x.XGetWindowProperty.argtypes=[ctypes.c_void_p,ctypes.c_ulong,ctypes.c_ulong,ctypes.c_long,ctypes.c_long,ctypes.c_int,ctypes.c_ulong,ctypes.POINTER(ctypes.c_ulong),ctypes.POINTER(ctypes.c_int),ctypes.POINTER(ctypes.c_ulong),ctypes.POINTER(ctypes.c_ulong),ctypes.POINTER(ctypes.c_void_p)]
        x.XFree.argtypes=[ctypes.c_void_p]
        self.d=x.XOpenDisplay(None)
        if not self.d: raise RuntimeError('no X display')
        self.w=x.XCreateSimpleWindow(self.d,x.XDefaultRootWindow(self.d),0,0,1,1,0,0,0)
        self.sel=x.XInternAtom(self.d,b'CLIPBOARD',0)
        self.prop=x.XInternAtom(self.d,b'PYCLIP',0)
        self.targets=[x.XInternAtom(self.d,b'UTF8_STRING',0),x.XInternAtom(self.d,b'STRING',0)]
    def get(self):
        x=self.x
        for target in self.targets:
            x.XConvertSelection(self.d,self.sel,target,self.prop,self.w,0); x.XFlush(self.d)
            end=time.time()+.7
            while time.time()<end:
                if x.XPending(self.d):
                    e=XE(); x.XNextEvent(self.d,ctypes.byref(e))
                    if e.type==31 and e.xselection.property:
                        at=ctypes.c_ulong(); fmt=ctypes.c_int(); n=ctypes.c_ulong(); left=ctypes.c_ulong(); p=ctypes.c_void_p()
                        x.XGetWindowProperty(self.d,self.w,self.prop,0,1000000,1,0,ctypes.byref(at),ctypes.byref(fmt),ctypes.byref(n),ctypes.byref(left),ctypes.byref(p))
                        if p:
                            ln=n.value if fmt.value==8 else n.value*(2 if fmt.value==16 else 4)
                            b=ctypes.string_at(p,ln); x.XFree(p)
...
def wait_key():
    try:
        ...
        try:
            if select.select([sys.stdin],[],[],0)[0]: return sys.stdin.read(1)
...
def clipqr():
    tui(1); last=None; xc=None
    try:
        try: xc=XClip()
        except Exception as e:
            show(None,'X11 clipboard unavailable: '+str(e)); time.sleep(2); return
        show(None,'watching X11 clipboard | copy text | Ctrl+C or q exit')
        while 1:
            t=xc.get()
            if t!=last:
                last=t
                if not t: show(None,'clipboard is empty | Ctrl+C or q exit')
                else:
                    try:
                        m,v,n=qt(t)
                        show(m,'clip QR v%d | %d bytes | %d chars | Ctrl+C or q exit'%(v,n,len(t)))
                    except Exception as e: show(None,str(e)+' | Ctrl+C or q exit')
            k=wait_key()
            if k in ('q','Q',chr(27)): break
            time.sleep(.3)
...
def usage():
    print('usage:')
    print('  python3 scqr_xclip_tui.py')
    print('  python3 scqr_xclip_tui.py -t TEXT')
    print('  echo TEXT | python3 scqr_xclip_tui.py -')
    print('  python3 scqr_xclip_tui.py FILE')
    print('  python3 scqr_xclip_tui.py --once FILE')
    print('  python3 scqr_xclip_tui.py --clear')

def run():
    a=sys.argv[1:]
    if not a: clipqr(); return
    once=0
